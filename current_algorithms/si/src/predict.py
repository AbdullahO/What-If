...
def diagnostic(pre_df, post_df, t=0.99, alpha=0.05):
    ...
    for i, iv in enumerate(ivs):
        ...
        for m, metric in enumerate(metrics):
            diagnostic_rslts[i * M + m, :] = diagnostic_test(
                pre_df, post_df, unit_ids, metric, iv, t=t, alpha=alpha
            )

    # create output dataframe
    df = pd.DataFrame(
        data=diagnostic_rslts, columns=["pvalues_test", "energy_statistic"]
    )
...
def fill_tensor(
    ...
    for iv in ivs:
        ...
        for unit in units:
            ...
            for metric in metrics:
                ...
                X2 = post_df.loc[
                    (post_df.unit.isin(donor_units))
                    & (post_df.intervention == iv)
                    & (post_df.metric == metric)
                ]
                ...
                idx_data = (
                    np.vstack([idx_data, [unit, iv, metric]])
                    if idx_data.size
                    else np.array([unit, iv, metric])
                )
    ...
    if return_donors_info:
        return df_synth, donors_dict
    return df_synth
